    ...
    return (
        <div className="border-gray-200 border-2 dark:border-zinc-800 dark:shadow-black p-3 bg-white rounded-md shadow-md dark:bg-zinc-800 dark:text-white">
            <ul className="space-y-4">
                {[
                    { label: "Capital:", value: `$${(data.balance ?? 0).toFixed(2)}` },
                    { label: "Beneficio promedio:", value: `$${(data.expectancy ?? 0).toFixed(2)}` },
                    ...
                    { label: "Coeficiente de beneficio:", value: (data.profitFactor ?? 0).toFixed(2) },
                ].map((item, index) => (
