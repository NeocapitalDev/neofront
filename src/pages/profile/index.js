...
import { CheckCircleIcon, XCircleIcon, CalendarIcon, AtSymbolIcon, PencilIcon } from "@heroicons/react/24/solid";
...
const ProfilePage = () => {
  ...
  return (
    <Layout>
      {/* Cabecera de perfil con efecto de gradiente sutil */}
      <div className="relative overflow-hidden bg-gradient-to-r from-white to-gray-50 dark:from-zinc-800 dark:to-zinc-900 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-700 transition-all">
        ...
        <div className="p-6 flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-xl font-semibold text-zinc-800 dark:text-white">Perfil de Usuario</h1>
          </div>

          <div className="flex items-center">
            <span className={`px-3 py-1 rounded-full text-xs font-medium ${isVerified ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' :
      ...
      <div className="mt-6 bg-white dark:bg-zinc-800/90 shadow-md rounded-xl border border-gray-100 dark:border-zinc-700 overflow-hidden transition-all">
        <div className="p-8 flex flex-col items-center relative">
          {/* Decorative curved design element */}
          ...
          <div className="flex items-center text-gray-500 dark:text-gray-400 text-sm mb-6">
            ...
            <span className="ml-1 font-medium">
              {data.createdAt ? new Date(data.createdAt).toLocaleDateString() : "No disponible"}
            </span>
          </div>
          ...
          <div className="w-full max-w-lg mt-2 px-6 py-4 bg-gray-50 dark:bg-zinc-700/30 rounded-lg border border-gray-100 dark:border-zinc-700/50">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <div className={`p-1 rounded-full ${isVerified ? 'bg-green-100 dark:bg-green-900/30' : 'bg-yellow-100 dark:bg-yellow-900/30'}`}>
                  {isVerified ? (
                ...
                <div>
                  <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">Estado de verificación</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {isVerified ? "Tu cuenta está completamente verificada" : "Completa la verificación para desbloquear todas las funciones"}
                  </p>
                </div>
              </div>

              {!isVerified && (
                <button className="text-xs px-3 py-1 rounded-md bg-[var(--app-primary)]/90 hover:bg-[var(--app-primary)] text-white transition-colors duration-200 font-medium">
                  Verificar
                </button>
              )}
            </div>
      ...
      {/* Formulario de perfil con diseño mejorado */}
      <form onSubmit={handleSubmit} className="mt-6 bg-white dark:bg-zinc-800/90 shadow-md rounded-xl border border-gray-100 dark:border-zinc-700 overflow-hidden transition-all">
        <div className="border-b border-gray-100 dark:border-zinc-700/50 p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <h2 className="text-lg font-semibold text-zinc-800 dark:text-white">Información Personal</h2>
            </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <div className={`relative ${isVerified ? 'opacity-80' : ''}`}>
                ...
                />
                {isVerified && <div className="absolute inset-0 bg-gray-200/10 dark:bg-zinc-900/10 rounded-md cursor-not-allowed"></div>}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="zipCode" className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                Código Postal
              </Label>
              <div className={`relative ${isVerified ? 'opacity-80' : ''}`}>
                <Input
          ...
          <div className="pt-4 mt-4 border-t border-gray-100 dark:border-zinc-700/50">
            ...
            >
              {loading ? (
                ...
                <span>{isVerified ? "Información verificada" : "Guardar cambios"}</span>
              )}
            </button>

            {isVerified && (
              <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                No es posible editar la información una vez que la cuenta ha sido verificada
              </p>
            )}
          </div>
        </div>
