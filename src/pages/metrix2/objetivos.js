...
export default function Objetivos({ challengeConfig, metricsData, initBalance, pase }) {
    ...
    useEffect(() => {
        ...
        if (metricsData.dailyGrowth && metricsData.dailyGrowth.length > 0) {
            maxDailyDrawdown = Math.abs(Math.min(
                ...metricsData.dailyGrowth.map(day => day.profit),
                0 // Incluir 0 para asegurar que sea negativo o 0
            ));
        }
