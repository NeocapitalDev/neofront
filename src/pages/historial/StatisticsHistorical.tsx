...
export default function StatisticsHistorical({ metadata }) {
  ...
  const initialCapital =
    typeof data.brokerInitialBalance === "number"
  ...
  const expectancy =
    data.expectancy !== undefined
      ? data.expectancy
      : (wonTradesPercent / 100) * avgWin -
        ((100 - wonTradesPercent) / 100) * Math.abs(avgLoss);

  // Usar el profit factor desde los datos o calcular una aproximación
  const profitFactor =
    data.profitFactor ||
    ((data.wonTrades || 0) * avgWin) /
      Math.abs((data.lostTrades || 1) * avgLoss || 1);
  ...
  return (
    <div className="border-gray-200 border-2 dark:border-zinc-900 dark:shadow-black bg-white rounded-md shadow-md dark:bg-zinc-900  dark:text-white">
          ...
          <li
            ...
            className={`flex flex-col px-6 py-4 ${
              index < metrics.length - 2
                ? "border-b border-gray-300 dark:border-zinc-500 w-full"
                : ""
            }`}
          >
