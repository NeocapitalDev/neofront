...
export function RiskManagementEquityTracking({
  ...
  const [trackerName, setTrackerName] = useState<string>('example-tracker');
  ...
  return (
    <div className="max-w-lg mx-auto p-6 bg-white dark:bg-gray-800 shadow-lg rounded-lg">
      <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Risk Management: Equity Tracking</h1>
      <form onSubmit={triggerToFetchData} onReset={reset} className="mt-4 space-y-4">
        <label className="block text-gray-700 dark:text-gray-300">Tracker Name</label>
        <input className="w-full p-2 border rounded bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white" value={trackerName} onChange={e => setTrackerName(e.target.value)} />
