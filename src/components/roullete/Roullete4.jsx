...
export default function RuletaSorteo({
  ...
  const drawWheel = (ctx, options, currentAngle) => {
    ...
    if (centerImageLoaded && centerImageRef.current) {
      // Dibuja un círculo negro como fondo para la imagen
      ctx.beginPath();
      ctx.arc(centerX, centerY, centerRadius, 0, 2 * Math.PI);
      ctx.fillStyle = "#000";
      ctx.fill();

      // Dibuja la imagen en el centro con rotación
      ctx.save();
      ctx.beginPath();
      ctx.arc(centerX, centerY, centerRadius, 0, 2 * Math.PI);
      ctx.closePath();
      ...
      ctx.translate(centerX, centerY);
      // Rotar continuamente basado en el ángulo actual
      ctx.rotate(currentAngle); // Velocidad de rotación más lenta que la ruleta
      ctx.translate(-centerX, -centerY);
